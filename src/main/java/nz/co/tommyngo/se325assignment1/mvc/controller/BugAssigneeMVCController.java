...
public class BugAssigneeMVCController {
	...
	/**
	 * Handler method to process URLs ending with /assignees/<id>, that are invoked by HTTP POST
	 * methods, and which are a query parameter named "form". The purpose of this method is to 
	 * update an existing and specified assignee based on form data that is submitted with the POST 
	 * request.
	 * @param assignee the assignee object, created by the MCV framework, that is represented by the
	 *        POSTed content.
	 ...
	 *        assignee object and populated the BindingResult if errors are detected. 
	 ...
	 *        constraints have failed, this method also adds the assignee object to the model so that
	...
	public String update(@Valid BugAssignee assignee, BindingResult bindingResult, Model uiModel, RedirectAttributes redirectAttributes) {
		...
		if(bindingResult.hasErrors()) {
			...
			// Add the assignee that has been POSTed (and edited incorrectly by the user) to the 
			// model.
			uiModel.addAttribute("assignee", assignee);
			
			// Add a Message to the model explaining that the assignee could not be saved due to
		...
		// Redirect the browser to a page that displays the updated assignee.
		return "redirect:/assignee/" + assignee.getId();
	}
	
	/**
	 * Handler method to process URLs ending with /assignees/<id>, that are invoked by HTTP GET
	 * methods, and which have a query parameter named "form". The purpose of this method is to
	 * retrieve a form that users can use to edit the specified assignee.
	 ...
	 * with the required assignee so that is can be included in the view.  
	 * @return logical view name.
	 */
	@RequestMapping(value = "/{id}", params = "form", method = RequestMethod.GET)
	public String updateForm(@PathVariable("id") Long id, Model uiModel) {
		// Lookup the assignee and add it to the model.
	...
	/**
	 * Similar to handler method update(), but enables new assignees to be created as opposed to 
	 * existing assignees being edited.
	 */
	...
	/**
	 * Similar to handler method updateForm(), but returns a form that allows new assignees to be
	...
	public String delete(@PathVariable("id") Long id, Model uiModel, RedirectAttributes redirectAttributes) {
		...
		assigneeDao.delete(assigneeDao.findAssigneeById(id));
